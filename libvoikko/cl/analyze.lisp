...
(defmethod free-foreign-resource ((object mor-analysis))
  (with-slots (address list) object
    (cffi:foreign-funcall "voikko_free_mor_analysis" :pointer address :void)
    (setf list nil)))

(defun analyze-word (instance word)
  (error-if-not-active-instance instance)
  (let ((address (cffi:foreign-funcall "voikkoAnalyzeWordCstr"
    ...
    (when (proper-pointer-p address)
      (make-instance 'mor-analysis :address address
                     :list (loop :for i :upfrom 0
                              :for a := (cffi:mem-aref address :pointer i)
                              :until (cffi:null-pointer-p a)
                              :collect a)))))

(defun mor-analysis-key-values (mor-analysis)
  (let ((keys-ptr (cffi:foreign-funcall "voikko_mor_analysis_keys"
                                        :pointer mor-analysis
                                        :pointer)))

    (when (proper-pointer-p keys-ptr)
      (loop :for i :upfrom 0
         :for key := (cffi:mem-aref keys-ptr :string i)
         :while (stringp key)
         :for value-ptr := (cffi:foreign-funcall "voikko_mor_analysis_value_cstr"
                                                 :pointer mor-analysis
                                                 :string key :pointer)

         :collect (cons key (cffi:foreign-string-to-lisp value-ptr))
         :do (cffi:foreign-funcall "voikko_free_mor_analysis_value_cstr"
